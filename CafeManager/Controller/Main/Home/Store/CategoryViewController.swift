...
class CategoryViewController: BaseViewController {
    ...
    func registerNIB() {
        tblCategories.register(UINib(nibName: CategoryTableViewCell.nibName, bundle: nil), forCellReuseIdentifier: CategoryTableViewCell.reuseIdentifier)
    }
    ...
    @IBAction func onAddPressed(_ sender: UIButton) {
        ...
        if categories.contains(where: {$0.categoryName == txtCategoryName.text!}) {
            displayErrorMessage(message: "The category already exists!", completion: nil)
            return
        }
...
extension CategoryViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tblCategories.dequeueReusableCell(withIdentifier: CategoryTableViewCell.reuseIdentifier, for: indexPath) as! CategoryTableViewCell
...
extension CategoryViewController : FirebaseActions {
    ...
    func onFoodCategoryNotAdded() {
        dismissProgress()
        displayErrorMessage(message: "Category not added!", completion: nil)
    }
    ...
    func onFoodCategoryNotRemoved() {
        dismissProgress()
        displayErrorMessage(message: "Category not removed!", completion: nil)
    }
}
